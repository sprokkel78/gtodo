...
status = subprocess.Popen("/usr/bin/ps aux | /usr/bin/grep \"python3 ./gtodo.py\" | /usr/bin/grep -v \"grep\"",\
...
def show_index_of_all_items(one, two, three, four, five):
    ...
    command = "/usr/bin/cat " + homedir + "/.gtodo/*.txt | sort > " + homedir + "/.gtodo/Index.txt"
    result = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    out = result.communicate()
    sleep(0.5)
    listbox_todo.remove_all()
...
def print_file(obj):
    ...
    printer_name = list(printers.keys())[0]  # Choose the first printer

    # Print a file
    file_path = homedir + "/.gtodo/" + topic + ".txt"
    conn.printFile(printer_name, file_path, "gTODO Print Job", {"media": "A4", "fit-to-page": "true"})
...
class MyApp(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        scrolled_window_1.set_size_request(300, 500)
        ...
        scrolled_window_topics.set_size_request(300,500)
        scrolled_window_topics.set_vexpand(True)
        ...
        box_statusbar = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
        ...
        scrolled_window.set_size_request(700, 500)
        global listbox_todo
        ...
        print_category = Gtk.Button(label="Print")
        print_category.set_size_request(150, -1)
        print_category.set_halign(1)
        print_category.connect("clicked", print_file)
        box_11a.append(print_category)
